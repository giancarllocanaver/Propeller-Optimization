...
beta = np.array([46.3,43.25,38.1,31.65,26.3,22.4,19.5,0])*np.pi/180.
...
df_resultados_aerof = pd.DataFrame({'Velocidade': [], 'RPM': [], 'Alpha': [], 'Re': [], 'Cl': [], 'Cd': [], 'Tipo': []})

for v in v_list:
    for rpm in rpm_list:
        eta, df_resultados_aerof = helice(aerofolios,v,1.789e-5,288.2,1.225,96*0.0254,2,raio,c,beta,rpm, solucao_viscosa=False, validacao=df_resultados_aerof).rodar_helice()

        df_resultados_helice = df_resultados_helice.append(pd.DataFrame({'Velocidade': [v], 'RPM': [rpm], 'Eficiencia': [eta]}), ignore_index=True)
