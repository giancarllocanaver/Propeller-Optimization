...
class DataValidation:
    ...
    def check_file_existance(self, directory: str) -> None:
        ...
        if not os.path.isfile(directory):
            raise FileNotFound(
                "There is no file or directory with the current name: {file}".format(file=directory)
            )
    ...
    def check_file_extension(self, file: str) -> None:
        ...
        if not extension == 'json':
            raise FileExtensionError(
                "The file in the directory {file} must be a json file!".format(file=file)
            )
    ...
    def check_data_dtypes(self, data: dict) -> None:
        ...
        error_data_types = [
            ...
            for first_key in INPUT_SCHEMA
            for second_key in INPUT_SCHEMA.get(first_key)
            if type(data.get(first_key).get(second_key)) != INPUT_SCHEMA.get(first_key).get(second_key)
        ]
        if error_data_types:
            ErrorDtypeKeys(
